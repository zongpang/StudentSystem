...
public class Localutil {
		...
		for (int i = 0; i < pageTotal; i++) {
			if (i == (pageNow - 1)) {
				...
				if (total == pageSize) {// 总页数等于每页条数
					...
					return arrayList;
				} else if (total < pageSize) {// 总页数小于每页条数
					for (int j = i * pageSize; j < i * pageSize + total; j++) {
						arrayList.add(objects.get(j));
					}
					return arrayList;
				} else {// 总条数大于每页条数
					if ((pageNow == total / pageSize + 1) && (total % pageSize != 0)) {// 最后一页
	 ...
	 * @return
	 */
	public static int pageDefine(ArrayList list, Integer pageSize, Integer pageNow) {
